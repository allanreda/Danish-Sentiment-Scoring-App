...
if section == "Analysér og plot længere tekst":
    ...
    if st.button("Analysér og plot tekst"):
        if user_input_2:
            ...
            # Truncate each sentence to the first 20 characters for x-tick labels
            shortened_sentences = [sentence[:20] for sentence in sentences]

            # Use the shortened sentences as x-tick labels
            plt.xticks(range(len(sentences)), shortened_sentences, rotation=45, ha="right")  # Rotate labels for better readability
